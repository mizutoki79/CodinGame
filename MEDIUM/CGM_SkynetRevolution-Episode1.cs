...
{   
    public class Node
    {
        internal int DistanceFromAgent {get; set;} = int.MaxValue;
        internal List<Node> Links {get; set;} = new List<Node>();
        internal bool Gateway {get; set;} = false; 
        ...
        public override string ToString(){
            return string.Format("[node.{0} distance: {1}]", indexValue, DistanceFromAgent);
        }
    }
    ...
    public static Tuple<int, int> CutLink ()
    {
        ...
        Console.Error.WriteLine("min distance: {0}", cutCandidateList.Min(elem2 => elem2.Item1.DistanceFromAgent));
        Tuple<Node, Node> cutNodes = cutCandidateList
         .First(elem1 => elem1.Item1.DistanceFromAgent == (cutCandidateList.Min(elem2 => elem2.Item1.DistanceFromAgent)));
